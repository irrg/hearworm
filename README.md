...
Supports mp3, m4b, m4a, aac, flac, ogg, wav, and wma as input or output. The primary use case is splitting or converting m4b audiobooks into mp3 files for devices that don't support the m4b format.
...
Batch-convert a directory of audio files to another format (e.g. m4b → mp3).

```bash
hearworm convert /path/to/audiobooks/ --output-dir ./mp3s/ --audio-format mp3
```

If source audio already matches the target codec, it is remuxed without re-encoding.
...
```bash
hearworm split input.m4b --output-dir ./chapters/ --audio-format mp3
```
...
```bash
hearworm merge /path/to/audio-files/ \
...
```bash
hearworm chapters input.m4b --output-file output.m4b
```
